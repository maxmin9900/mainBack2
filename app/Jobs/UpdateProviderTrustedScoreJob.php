...
class UpdateProviderTrustedScoreJob implements ShouldQueue
{
        ...
        try {
            ...
            if ($user && $user->level == 2) {
                ...
                if ($service) {
                    ...
                    $trustedScore = $this->data['scTotal'] / $this->data['scCount'];
                    $service->update([
                        'trustedScore' => $trustedScore
                    ]);
                    $user->updateTrustedScore();
                } else {
            ...
            Log::info("ERROR IN PROVIDER {$this->data['provider']} : " . $exception->getMessage());
        }
    }
}
